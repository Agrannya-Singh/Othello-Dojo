import { TIMING, TOAST_MESSAGES, ANALYSIS } from "@/lib/constants/game";
...
export default function Home() {
  ...
  const updateGameData = useCallback((currentBoard: BoardState, player: Player) => {
    ...
    if (newValidMoves.length === 0) {
      ...
      if (opponentMoves.length === 0) {
        ...
        toast({
          title: TOAST_MESSAGES.TURN_SKIPPED,
          description: TOAST_MESSAGES.TURN_SKIPPED_DESC(player, opponent),
        });
  ...
  const handlePlayerMove = async (move: Move) => {
    ...
    setTimeout(() => setFlippingPieces([]), TIMING.ANIMATION_DURATION);

    playSound('place');
    if (flipped.length > 0) {
      setTimeout(() => playSound('flip'), TIMING.SOUND_DELAY);
    }
  ...
  const handleSuggestMove = () => {
    ...
    try {
      ...
      if (move) {
        ...
        toast({
          title: TOAST_MESSAGES.AI_SUGGESTION,
          description: TOAST_MESSAGES.AI_SUGGESTION_DESC(moveString),
        });
      } else {
         toast({
          title: TOAST_MESSAGES.NO_SUGGESTION,
          description: TOAST_MESSAGES.NO_SUGGESTION_DESC,
          variant: "destructive",
        });
      }
    } catch (error) {
      ...
      toast({
        ...
        description: TOAST_MESSAGES.SUGGESTION_ERROR,
        variant: "destructive",
      });
    } finally {
